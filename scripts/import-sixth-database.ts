...
async function importSixthDatabase() {
  console.log('\n🚀 Starting import of finalsixthdatabase.xlsx...\n');
  console.log('='.repeat(60));

  const filePath = path.resolve(process.cwd(), 'finalsixthdatabase.xlsx');
  
  if (!fs.existsSync(filePath)) {
    console.error(`❌ File not found: finalsixthdatabase.xlsx`);
    process.exit(1);
  }

  console.log(`📖 Reading finalsixthdatabase.xlsx...`);
...
// Run import
importSixthDatabase().catch((error) => {
  console.error('\n❌ Fatal error:', error);
  process.exit(1);
});
