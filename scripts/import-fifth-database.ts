...
async function importFifthDatabase() {
  console.log('\n🚀 Starting import of finalfifthdatabase.xlsx...\n');
  console.log('='.repeat(60));

  const filePath = path.resolve(process.cwd(), 'finalfifthdatabase.xlsx');
  
  if (!fs.existsSync(filePath)) {
    console.error(`❌ File not found: finalfifthdatabase.xlsx`);
    process.exit(1);
  }

  console.log(`📖 Reading finalfifthdatabase.xlsx...`);
...
// Run import
importFifthDatabase().catch((error) => {
  console.error('\n❌ Fatal error:', error);
  process.exit(1);
});
