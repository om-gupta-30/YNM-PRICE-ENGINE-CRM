...
function getStateAndCity(rawState: string, rawCity: string): { state: string; city: string } {
  ...
  // If state is empty, try to use city or default to 'Other'
  if (!stateTrimmed) {
    // If city looks like a state name, use it
    if (cityTrimmed && !/^\d{6}$/.test(cityTrimmed)) {
      // Check if city is actually a state name
      const possibleStates = ['Delhi', 'Mumbai', 'Gurgaon', 'Karnataka', 'Tamil Nadu', 'Maharashtra', 'Gujarat', 'Uttar Pradesh', 'Haryana', 'Punjab', 'Rajasthan', 'West Bengal', 'Telangana', 'Andhra Pradesh', 'Kerala', 'Odisha', 'Madhya Pradesh', 'Bihar', 'Jharkhand', 'Uttarakhand', 'Himachal Pradesh', 'Assam', 'Chhattisgarh'];
      if (possibleStates.includes(cityTrimmed)) {
        return { state: cityTrimmed, city: 'Other' };
      }
      return { state: 'Other', city: cityTrimmed };
    }
...
async function getOrCreateState(stateName: string): Promise<number> {
  if (!stateName || stateName === 'Other') {
    // Try to find 'Other' state, or create it
    ...
    // Create 'Other' state if it doesn't exist
    const { data: newState, error: createError } = await supabase
      .from('states')
      .insert({ state_name: 'Other' })
      .select('id')
      .single();
    
    if (createError) {
      throw new Error(`Failed to create state "Other": ${createError.message}`);
    }
    
    console.log(`   🆕 Created new state: Other`);
    return newState.id;
  }
...
async function importData() {
  console.log('\n🚀 Fixing and re-importing data from seconddatabase.xlsx...\n');
  ...
  for (const [key, accountData] of accountMap.entries()) {
    try {
      ...
      // Get or create state and city (now handles missing state)
      ...
      // Create contacts - ensure ALL contacts are imported
      for (const contact of accountData.contacts) {
        if (!contact.name) continue;
        
        // Check if contact already exists (more lenient check)
