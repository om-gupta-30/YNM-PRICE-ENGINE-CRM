...
export default function ImportDatabase1Page() {
  ...
  const handleImport = async () => {
    ...
    try {
      const response = await fetch('/api/admin/import-database1', {
        method: 'POST',
      });
      ...
      <div className="relative z-10 p-6">
        <div className="max-w-4xl mx-auto">
          <div className="mb-6">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">Import Database1.xlsx</h1>
            <p className="text-slate-400">
              Import accounts, sub-accounts, and contacts from database1.xlsx file
            </p>
          </div>

          <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50 mb-6">
            <h2 className="text-xl font-bold text-white mb-4">Import Information</h2>
            <div className="space-y-2 text-slate-300 text-sm">
              <p>This import will process the following fields:</p>
              <ul className="list-disc list-inside space-y-1 ml-4">
                <li>Account Name, Company Stage, Company Tag</li>
                <li>Industry and Sub-Industry of Account</li>
                <li>Sub Account Name, Address, State, City, PIN Code</li>
                <li>Contact Name, Contact Number, Designation, Email</li>
              </ul>
              <p className="mt-4 text-amber-300">
                ⚠️ Note: The file must be named "database1.xlsx" and placed in the project root directory.
              </p>
            </div>
          </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-white">Preview Data</h2>
              <button
                onClick={async () => {
                  try {
                    const response = await fetch('/api/admin/import-database1/preview');
                    const data = await response.json();
                    if (data.success) {
                      alert(`✅ File found!\n\nTotal rows: ${data.totalRows}\nHeaders: ${data.headers.join(', ')}\n\nCheck console for sample data.`);
                      console.log('Sample rows:', data.sampleRows);
                    } else {
            ...
            <p className="text-slate-400 text-sm">Click to verify the Excel file can be read before importing</p>
          </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50">
            <button
              ...
              disabled={loading}
              className="w-full px-6 py-3 bg-gradient-to-r from-premium-gold to-amber-600 hover:from-amber-600 hover:to-premium-gold disabled:from-slate-600 disabled:to-slate-600 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-all duration-300"
            >
            ...
            {result && (
              <div className={`mt-6 p-4 rounded-lg border ${
                ...
                {result.success ? (
                  <div>
                    <h3 className="text-green-300 font-bold mb-3">✅ Import Successful!</h3>
                    {result.summary && (
                      <div className="space-y-2 text-slate-200 text-sm">
                        <p>Accounts Created: {result.summary.accountsCreated}</p>
                        <p>Accounts Updated: {result.summary.accountsUpdated}</p>
                        <p>Sub-Accounts Created: {result.summary.subAccountsCreated}</p>
                        <p>Sub-Accounts Updated: {result.summary.subAccountsUpdated}</p>
                        <p>Contacts Created: {result.summary.contactsCreated}</p>
                        <p className="mt-2 font-semibold">Total Accounts Processed: {result.summary.totalAccounts}</p>
                      </div>
                    )}
                    {result.errors && result.errors.length > 0 && (
                      <div className="mt-4">
                        <p className="text-amber-300 font-semibold mb-2">⚠️ Some errors occurred:</p>
                  ...
                  <div>
                    <h3 className="text-red-300 font-bold mb-2">❌ Import Failed</h3>
                    <p className="text-slate-200">{result.error || 'Unknown error occurred'}</p>
                    {result.errors && result.errors.length > 0 && (
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-700/50 mt-6">
            ...
            {dataLoading ? (
              ...
              <div className="text-center py-8 text-slate-400">
                {importedData ? 'No data found matching your search.' : 'No data imported yet. Click "Start Import" to import data from database1.xlsx'}
              </div>
            ) : (
