...
export default function ThrieBeamPage() {
  ...
  const isQuotationComplete = isAdmin || (stateId !== null && 
                               cityId !== null && 
                               quotationDate.trim() !== '' && 
                               estimateDate !== null &&
                               expiryDate !== null &&
  ...
  useEffect(() => {
    const lookupHistoricalPricing = async () => {
      ...
      const hasThrieBeamSpecs = includeThrieBeam && thrieBeam.thickness && thrieBeam.coatingGsm;
      ...
      if (!hasThrieBeamSpecs && !hasPostSpecs && !hasSpacerSpecs) return;
      
      setIsLookingUpHistory(true);
      
      try {
        const match = await lookupHistoricalMBCBQuote({
          wBeamThickness: thrieBeam.thickness ?? undefined,
          wBeamCoating: thrieBeam.coatingGsm ?? undefined,
    ...
    includeThrieBeam, thrieBeam.thickness, thrieBeam.coatingGsm,
  ...
  const handleGetAISuggestion = async () => {
    ...
    if (includeThrieBeam) {
      productSpecs.thrieBeamThickness = `${thrieBeam.thickness}mm`;
      productSpecs.thrieBeamCoating = `${thrieBeam.coatingGsm} GSM`;
    }
