...
export default function ThrieBeamPage() {
  ...
  const generatePDF = async () => {
    ...
    if (includeThrieBeam && thrieBeamResult?.found) {
      parts.push({
        partName: 'Thrie Beam',
        thickness: thrieBeam.thickness,
        length: null,
        coatingGsm: thrieBeam.coatingGsm,
        blackMaterialWeight: thrieBeamResult.weightBlackMaterial,
        zincAddedWeight: thrieBeamResult.weightZincAdded,
        totalWeight: thrieBeamResult.totalWeight,
      });
    }
    ...
    if (includeThrieBeam && thrieBeamResult?.found) multipliers.wBeam = 1; // Using wBeam key for Thrie Beam
    if (includePost && postResult?.found) multipliers.post = 2;
    if (includeSpacer && spacerResult?.found) multipliers.spacer = 2;
    ...
    // PDF export disabled
    // await generateProfessionalPDF(pdfData, `Thrie-Beam-Estimate-${new Date().toISOString().split('T')[0]}.pdf`);
