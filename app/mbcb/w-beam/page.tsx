...
export default function WBeamPage() {
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (data.estimateNumber) {
              ...
              console.warn('No estimate number returned from API');
              // Don't show error toast, just log it
            }
          } else {
            const errorData = await response.json().catch(() => ({ error: 'Unknown error' }));
            console.error('Failed to fetch estimate number:', errorData);
  ...
  const calculateFastenerWeight = (): number => {
    if (fastenerMode === 'default') {
      return 2; // Default: 2 kg for Single W-Beam
    } else {
  ...
  const updateOverallTotals = () => {
    ...
    // totalSetWeightKg = (1 × wBeamWeight) + (2 × postWeight) + (2 × spacerWeight) + fastenersWeight
  ...
  const costCalculations = useMemo(() => {
    ...
    if (fastenerMode === 'manual' && debouncedRatePerKg && debouncedRatePerKg > 0) {
      // Manual mode: Only fasteners are included, no parts
      // Use total fastener weight (not per rm) for cost calculation
      const totalFastenerWeight = calculateFastenerWeight();
      
      // Fastener material cost = Total Fastener Weight × Rate per kg
      fastenerMaterialCostPerRm = totalFastenerWeight > 0 ? totalFastenerWeight * debouncedRatePerKg : null;
      
      // Fastener transport cost (if transportation is enabled) = Total Fastener Weight × Transport Cost per kg
      fastenerTransportCostPerRm = includeTransportation && totalFastenerWeight > 0 
        ? totalFastenerWeight * transportCostPerKgValue 
        : null;
      
      // Total material cost = only fasteners (no parts)
      materialCostPerRm = fastenerMaterialCostPerRm || null;
      
      // Total transport cost = only fasteners (no parts)
      transportCostPerRm = fastenerTransportCostPerRm || null;
    } else {
  ...
  const handleSaveQuotation = async () => {
    ...
    try {
      ...
      const quotePayload = {
        ...
        customer_name: customerName || subAccountName, // Contact name for quotation
  ...
  // PDF Generation Function - Using New Professional PDF Generator
  const generatePDF = async () => {
    const currentUsername = username || (typeof window !== 'undefined' ? localStorage.getItem('username') || 'Admin' : 'Admin');
    
    // Fetch estimate number if not already set
    let currentEstimateNumber = estimateNumber;
    if (!currentEstimateNumber) {
      try {
        ...
        if (response.ok) {
          ...
          setEstimateNumber(currentEstimateNumber);
        } else {
          setToast({ 
            message: 'Failed to fetch estimate number. Please try again.', 
            type: 'error' 
          });
          return;
        }
        ...
        console.error('Error fetching estimate number:', error);
        setToast({ 
          message: 'Error fetching estimate number. Please try again.', 
          type: 'error' 
        });
        return;
      }
    ...
    if (!estimateDate || !expiryDate || !termsAndConditions.trim() || !currentEstimateNumber) {
      setToast({ 
        message: 'Please ensure estimate date, expiry date, terms & conditions are filled and quotation is confirmed', 
        type: 'error' 
      });
      return;
    }
    ...
    if (!finalTotal || finalTotal <= 0) {
      setToast({ 
        message: 'Please ensure all costs and quantities are confirmed', 
        type: 'error' 
      });
      return;
    }
    ...
    const placeOfSupplyText = stateName && cityName ? `${stateName}, ${cityName}` : '';
    const isTelanganaOrder = stateName.toLowerCase().includes('telangana');
    const gstRate = getGSTRate('MBCB', 'W-Beam Section');
    
    // Add main item with all parts combined
    if (materialCostPerRm && finalTotal) {
      const itemTotal = finalTotal;
      const taxAmount = itemTotal * (gstRate / (100 + gstRate));
      const subtotalBeforeTax = itemTotal - taxAmount;
      
      // Build description with all parts
      const partDescriptions: string[] = [];
      if (includeWBeam && wBeamResult?.found) {
        partDescriptions.push(`W-Beam: ${wBeam.thickness}mm, ${wBeam.coatingGsm}GSM, ${wBeamResult.totalWeight?.toFixed(3)}kg/rm`);
      }
      if (includePost && postResult?.found) {
        partDescriptions.push(`Post: ${post.thickness}mm, ${post.length}mm, ${post.coatingGsm}GSM, ${postResult.totalWeight?.toFixed(3)}kg/rm`);
      }
      if (includeSpacer && spacerResult?.found) {
        partDescriptions.push(`Spacer: ${spacer.thickness}mm, ${spacer.length}mm, ${spacer.coatingGsm}GSM, ${spacerResult.totalWeight?.toFixed(3)}kg/rm`);
      }
      
      items.push({
        itemName: 'Metal Beam Crash Barriers (MBCB)',
        description: partDescriptions.join(' | '),
        dimensions: `Total Weight: ${totalWeight?.toFixed(3)} kg/rm`,
        hsnSacCode: getHSNCode('MBCB', 'W-Beam Section'),
        quantity: quantityRm || 1,
        rate: subtotalBeforeTax / (quantityRm || 1),
        ...(isTelanganaOrder 
          ? { igst: gstRate, amount: itemTotal }
          : { sgst: gstRate / 2, cgst: gstRate / 2, amount: itemTotal }
        ),
      });
    }
    ...
    if (items.length === 0) {
      setToast({ 
        message: 'No items to generate PDF. Please ensure material cost and quantity are confirmed.', 
        type: 'error' 
      });
      return;
    }
    ...
    const totalTax = items.reduce((sum, item) => {
      return sum + (item.amount - (item.rate * item.quantity));
    }, 0);
    
    const finalTotalWithTax = finalTotal || (subtotal + totalTax);
    
    // Prepare new PDF data
    const pdfDataNew: PDFQuotationDataNew = {
      ...
      billTo: undefined, // To be filled from database later
      shipTo: undefined, // To be filled from database later
      items,
      subtotal,
      totalTax,
      finalTotal: finalTotalWithTax,
      totalInWords: numberToWords(finalTotalWithTax),
      termsAndConditions,
    ...
    try {
      ...
      await generateYNMESTPDF({
        ...
        subtotal,
        igstAmount: isTelanganaOrder ? totalTax : undefined,
        sgstAmount: !isTelanganaOrder ? totalTax / 2 : undefined,
        cgstAmount: !isTelanganaOrder ? totalTax / 2 : undefined,
        grandTotal: finalTotalWithTax,
        totalInWords: numberToWords(finalTotalWithTax),
        termsAndConditions,
        billTo: undefined,
        shipTo: undefined,
    ...
    <div className="min-h-screen flex flex-col items-start py-12 pt-16 pb-32 relative">
      <div className="w-full max-w-6xl mx-auto px-4">
        ...
        <div className="glassmorphic-premium rounded-3xl p-12 animate-fade-up card-hover-gold mb-10 card-3d card-depth" style={{ overflow: 'visible', position: 'relative', zIndex: 1 }}>
          ...
          <div className="mb-6">
            <StateCitySelect
              ...
              onStateChange={(newStateId, newStateName) => {
                ...
                setCityName(''); // Reset city name
                setIsQuotationConfirmed(false); // Reset confirmation when state changes
                setIsEditingQuotation(false);
              }}
              onCityChange={(newCityId, newCityName) => {
                setCityId(newCityId);
                setCityName(newCityName);
                setIsQuotationConfirmed(false); // Reset confirmation when city changes
                setIsEditingQuotation(false);
              }}
              required
          ...
          {/* Step 2: Dates - Show after Place of Supply is selected, always visible once shown */}
          {showDates && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 animate-fade-up" style={{ transform: 'none', backdropFilter: 'none', filter: 'none', mixBlendMode: 'normal' }}>
              <div style={{ transform: 'none', backdropFilter: 'none', filter: 'none', mixBlendMode: 'normal' }}>
                ...
                <DatePicker
                  ...
                  onChange={(date: Date | null) => {
                    ...
                    setIsQuotationConfirmed(false); // Reset confirmation when estimate date changes
                    setIsEditingQuotation(false);
                  }}
              ...
              <div style={{ transform: 'none', backdropFilter: 'none', filter: 'none', mixBlendMode: 'normal' }}>
                ...
                <DatePicker
                  ...
                  onChange={(date: Date | null) => {
                    setExpiryDate(date);
                    setIsQuotationConfirmed(false); // Reset confirmation when expiry date changes
                    setIsEditingQuotation(false);
                  }}
                  dateFormat="dd/MM/yyyy"
                  className="input-premium input-focus-glow w-full px-4 py-3 text-white placeholder-slate-400"
              ...
              <div>
                ...
                <SubAccountSelect
                  ...
                  onChange={(selectedId, subName) => {
                    ...
                    setContactId(null);
                    setCustomerName('');
                    setIsQuotationConfirmed(false); // Reset confirmation when sub-account changes
                    setIsEditingQuotation(false);
                  }}
                  employeeUsername={username || ''}
                  accountId={accountId}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setIsQuotationConfirmed(false); // Reset confirmation when purpose changes
                    setIsEditingQuotation(false);
                  }}
            ...
            <div className="mt-6 animate-fade-up">
              ...
              <textarea
                ...
                onChange={(e) => {
                  ...
                  setIsQuotationConfirmed(false); // Reset confirmation when terms change
                  setIsEditingQuotation(false);
                }}
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-col space-y-3">
                <label className="flex items-center space-x-3 cursor-pointer group">
                  ...
                  />
                  <span className="text-slate-200 font-semibold group-hover:text-premium-gold transition-colors">Use Default Fastener Weight (2 kg)</span>
                </label>
                <label className="flex items-center space-x-3 cursor-pointer group">
                  <input
                    type="radio"
                ...
                <input
                  ...
                  value={ratePerKg || ''}
                        onChange={(e) => {
                          ...
                          setIsCostConfirmed(false); // Reset confirmation when rate changes
                    ...
                    <label className="block text-sm font-semibold text-slate-200 mb-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setIsCostConfirmed(false); // Reset confirmation when transport cost changes
                          setIsEditingCost(false);
                        }}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setIsCostConfirmed(false); // Reset confirmation when installation cost changes
                          setIsEditingCost(false);
                        }}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setIsQuantityConfirmed(false); // Reset confirmation when quantity changes
                          setIsEditingQuantity(false);
                        }}
                        placeholder="Enter quantity"
                        className="input-premium input-focus-glow w-full px-4 py-3 text-white placeholder-slate-400"
                        disabled={!isCostConfirmed || (isQuantityConfirmed && !isEditingQuantity)}
                      />
                    </div>
