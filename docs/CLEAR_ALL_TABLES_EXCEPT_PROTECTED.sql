-- ============================================
-- CLEAR ALL TABLES EXCEPT PROTECTED TABLES
-- ============================================
...
DECLARE
    r RECORD;
    protected_tables TEXT[] := ARRAY['industries', 'sub_industries', 'cities', 'states', 'users'];
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'accounts') THEN
        SELECT COUNT(*) INTO user_count FROM accounts;
        RAISE NOTICE 'accounts: % rows', user_count;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'customers') THEN
        SELECT COUNT(*) INTO user_count FROM customers;
        RAISE NOTICE 'customers: % rows', user_count;
    END IF;
    
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'quotes') THEN
        SELECT COUNT(*) INTO user_count FROM quotes;
        RAISE NOTICE 'quotes: % rows', user_count;
    END IF;
