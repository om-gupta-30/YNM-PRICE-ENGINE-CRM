...
-- FIXED: Uses city_name column (confirmed by user)
...
-- Create states if they don't exist (using 'state_name' column)
...
-- Create cities if they don't exist (using 'city_name' column)
DO $$
DECLARE
    mh_id INTEGER := (SELECT id FROM states WHERE state_name = 'Maharashtra' LIMIT 1);
    gj_id INTEGER := (SELECT id FROM states WHERE state_name = 'Gujarat' LIMIT 1);
    ka_id INTEGER := (SELECT id FROM states WHERE state_name = 'Karnataka' LIMIT 1);
    dl_id INTEGER := (SELECT id FROM states WHERE state_name = 'Delhi' LIMIT 1);
    ts_id INTEGER := (SELECT id FROM states WHERE state_name = 'Telangana' LIMIT 1);
BEGIN
...
SELECT 
    ...
    (
        ...
        WHERE s.state_name = state_name_val
