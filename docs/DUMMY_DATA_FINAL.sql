...
-- FIXED VERSION: Dynamically gets or creates states/cities
...
-- Create states if they don't exist (use 'name' column, not 'state_name')
...
-- Create cities if they don't exist (linked to states)
-- Note: cities table uses 'name' column (same as states), states table uses 'name' column
INSERT INTO cities (name, state_id)
SELECT name, state_id FROM (
    VALUES
    ('Mumbai', (SELECT id FROM states WHERE name = 'Maharashtra' LIMIT 1)),
    ('Pune', (SELECT id FROM states WHERE name = 'Maharashtra' LIMIT 1)),
    ('Nashik', (SELECT id FROM states WHERE name = 'Maharashtra' LIMIT 1)),
    ('Ahmedabad', (SELECT id FROM states WHERE name = 'Gujarat' LIMIT 1)),
    ('Surat', (SELECT id FROM states WHERE name = 'Gujarat' LIMIT 1)),
    ('Bangalore', (SELECT id FROM states WHERE name = 'Karnataka' LIMIT 1)),
    ('New Delhi', (SELECT id FROM states WHERE name = 'Delhi' LIMIT 1)),
    ('Hyderabad', (SELECT id FROM states WHERE name = 'Telangana' LIMIT 1))
) AS v(name, state_id)
...
SELECT 
    ...
    (SELECT id FROM cities WHERE name = city_name_val AND state_id = (SELECT id FROM states WHERE name = state_name_val LIMIT 1) LIMIT 1) as city_id,
    contact_person,
    phone,
    email,
    address,
...
JOIN cities c ON c.name = v.city_name AND c.state_id = s.id
JOIN sub_accounts sa ON sa.sub_account_name = v.sub_account_name
WHERE NOT EXISTS (
