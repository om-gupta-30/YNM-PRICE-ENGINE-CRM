-- =====================================================
-- FINAL DUMMY DATA SCRIPT FOR YNM SAFETY CRM & PRICE ENGINE
-- This script creates comprehensive dummy data linking CRM and Price Engine
-- Uses: state_name for states, city_name for cities
-- =====================================================
...
-- =====================================================
-- STEP 1: CREATE STATES (if they don't exist)
-- =====================================================
...
WHERE NOT EXISTS (SELECT 1 FROM states WHERE states.state_name = v.state_name);

-- =====================================================
-- STEP 2: CREATE CITIES (if they don't exist)
-- =====================================================
...
FROM (VALUES
    ...
    ('Delhi', 'New Delhi'),
    ('Telangana', 'Hyderabad')
) AS v(state_name_val, city_name)
JOIN states s ON s.state_name = v.state_name_val
WHERE NOT EXISTS (
    SELECT 1 FROM cities c 
    WHERE c.city_name = v.city_name 
    AND c.state_id = s.id
);
...
SELECT 
    ...
    company_stage,
    company_tag,
    (SELECT id FROM states WHERE state_name = state_name_val LIMIT 1) as state_id,
    (
        SELECT c.id FROM cities c
        JOIN states s ON s.id = c.state_id
        WHERE s.state_name = state_name_val AND c.city_name = city_name_val
        LIMIT 1
    ) as city_id,
    contact_person,
    phone,
...
JOIN states s ON s.state_name = v.state_name
...
-- =====================================================
-- STEP 7: CREATE QUOTATIONS (SIGNAGES)
-- =====================================================
...
-- =====================================================
-- STEP 8: CREATE QUOTATIONS (PAINT)
-- =====================================================
...
-- =====================================================
-- STEP 9: UPDATE ENGAGEMENT SCORES
-- =====================================================
