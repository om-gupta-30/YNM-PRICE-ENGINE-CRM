...
const BackButton = memo(function BackButton({ href, label }: BackButtonProps) {
  ...
  return (
    <button
      onClick={handleBack}
      className="fixed top-4 left-4 z-[10000] inline-flex items-center glassmorphic-premium rounded-xl px-6 py-3 text-sm font-semibold text-white hover:text-premium-gold transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-premium-gold/30 group relative overflow-hidden btn-ripple btn-press btn-3d"
      style={{
