...
export default function LeadsDashboard({ leads, loading }: LeadsDashboardProps) {
  ...
  return (
    <div className="space-y-6 mb-6">
      ...
      {/* Status Counts - 6 Small Boxes */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {Object.entries(displayAnalytics.leadsByStatus).map(([status, count]) => (
          <div 
            key={status}
            className="glassmorphic-premium rounded-xl p-4 slide-up card-hover-gold border-2 border-premium-gold/20 shadow-lg"
          >
            <p className="text-xs text-slate-400 mb-1 truncate">{status}</p>
